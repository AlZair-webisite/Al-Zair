...
export function TrustBar() {
  return (
    <section className="border-b border-[#d3cabb] bg-[#f5f0e7]" aria-label="Why shop with us">
      <div className="mx-auto grid max-w-[1240px] divide-y divide-[#d3cabb] px-5 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4 lg:px-8">
        {items.map(([title, copy, Icon]) => (
          <div key={title} className="flex items-center gap-4 px-3 py-5 sm:px-5 lg:py-7">
            <Icon size={22} strokeWidth={1} className="shrink-0 text-[#a9823b]" />
            <div>
              <h2 className="text-[9px] font-bold uppercase tracking-[.12em]">{title}</h2>
              <p className="mt-1 max-w-[180px] text-[10px] leading-4 text-[#5e5a52]">{copy}</p>
            </div>
          </div>
        ))}
      </div>
